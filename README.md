...
- Дополнительный флаг `DAILY_LOSS_CAP_AUTO_HOLD=1` переводит бота в HOLD при
  превышении лимита. Переход выполняется системно (`AUTO_HOLD_DAILY_LOSS`),
  пишется аудит, отправляется оповещение и на `/ui/dashboard` появляется красный
  бейдж «AUTO-HOLD: DAILY LOSS CAP» с краткой расшифровкой. Двух-операторный
  процесс RESUME для людей остаётся неизменным.
...
- Фоновый `AutopilotGuard` раз в несколько секунд проверяет критические риски и
  автоматически отключает автоторговлю (`auto_loop=OFF`), если торговля ведётся в
  небезопасной среде. Период регулируется переменной окружения
  `AUTOPILOT_GUARD_INTERVAL_SEC` (по умолчанию — 5 секунд).
- Триггеры: переход дневного лимита из `OK` в `BREACH` и перевод биржевого
  watchdog'а в состояние `AUTO_HOLD`. В обоих случаях guard вызывает штатный
  контрол `hold_loop()` — автоторговля останавливается без глобального HOLD и
  без обхода RBAC/двух-операторного процесса.
- Каждое автоматическое отключение пишется в `audit_log` с записью
  `action=AUTO_TRADE_OFF`, `operator=system` и `reason`
  (`DAILY_LOSS_BREACH` или `WATCHDOG_AUTO_HOLD`). Повторные проверки не создают
  дублирующих записей.
- Логика идентична в режиме `DRY_RUN_MODE`: guard выключает автоторговлю и
  оставляет подробные детали в аудите, но реальные ордера не отправляются.
...
`GET /api/ui/watchdog_status` (тот же bearer-токен, что и для остальных `/api/ui/*`
ручек) возвращает `overall_ok` и словарь `exchanges` с полями `ok`,
`last_check_ts`, `reason` по каждой площадке. Роли `viewer` и `auditor` имеют
read-only доступ, `operator` видит тот же JSON. На `/ui/dashboard` есть
компактный блок **Exchanges Health** с той же информацией.

Фоновый вотчдог включается флагом `WATCHDOG_ENABLED=true`; период пингов задаёт
`WATCHDOG_INTERVAL_SEC`. При необходимости можно включить автоматический HOLD
(`WATCHDOG_AUTO_HOLD=true`) и короткие уведомления о переходах
(`NOTIFY_WATCHDOG=true`). Без этих флагов компонент работает как
мониторинговый источник правды о подключённых биржах.
...
### Journal & Restart-Resume (`FEATURE_JOURNAL`)

- При выставлении `FEATURE_JOURNAL=1` журнал действий пишется в ту же SQLite,
  что и основной `ledger` (таблица `order_journal`). Каждая запись несёт
  `uuid`, `ts`, `type` и `payload` и вставляется строго один раз. Повторная
  попытка с тем же `uuid` вернёт сохранённый payload без дублей.
- `/api/ui/cancel_all` принимает `correlation_id`. Повторный вызов с тем же
  значением не трогает биржу, а в журнал попадает событие
  `cancel_all.duplicate` с исходным результатом. Это защищает kill switch и
  Telegram-команды от повторной отмены одного и того же батча ордеров.
- При старте инстанс поднимает открытые ордера и позиции из `ledger`, помечает
  их как `RESUMED` и пишет событие `restart.resume` в журнал. Runtime получает
  актуальный снимок, поэтому автоторговля не попытается дозаявить уже
  существующие ордера.
- `GET /healthz` дополнен полями `journal_ok` (читает/пишет в журнал) и
  `resume_ok` (отражает успешную инициализацию `restart.resume`). Оба флага
  интегрированы в итоговый `ok`.
