...
### Exchange watchdog (draft)

`GET /api/ui/exchange_health` (bearer-токен тот же, что и для остальных `/api/ui/*`
ручек) возвращает агрегированное состояние подключённых бирж: флаг доступности,
индикатор rate limit'а, метку времени последнего успешного запроса и текст
последней ошибки, если она была. Роль `viewer` может читать этот срез в режиме
read-only, `operator` видит тот же JSON.

Эта версия — только мониторинг: никакие автоматические стопы торговли не
подвязываются к watchdog, поэтому решения о HOLD/RESUME и ручном вмешательстве
остаётся принимать операторам.
