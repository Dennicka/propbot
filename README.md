...
### Проверка System Status через API

Для быстрой проверки состояния сервиса выполните:

```bash
curl -s http://127.0.0.1:8000/api/ui/status/overview | jq '{overall, alerts}'
```

Поле `overall` показывает агрегированное состояние всех P0–P3 компонентов: `OK` — всё штатно, `WARN` — требуется внимание, `ERROR` — выявлена ошибка, `HOLD` — активирован стоп или SAFE_MODE. Массив `alerts` содержит активные алерты: `severity` отражает приоритет (от `warn` до `critical`), `title` и `msg` описывают метрику или компонент, а `since` показывает момент, когда условие сработало. Каждая запись привязана к `component_id`, что помогает быстро открыть нужный блок в UI.

Отдельные панели доступны по `GET /api/ui/status/components` и `GET /api/ui/status/slo` — они возвращают структуру компонентов и численные метрики/пороги SLO соответственно.
