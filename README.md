...
- `GET /api/ui/ops_report` — компактный JSON-срез с текущими SAFE_MODE/HOLD и
  DRY_RUN флагами, статусами стратегий (enabled/frozen + причина), экспозицией
  из `positions_store`, открытыми/partial хеджами, Unrealised PnL и последними
  критическими действиями оператора (hold/resume/disable/unfreeze и т.д.).
- `GET /api/ui/ops_report.csv` — тот же отчёт в плоском CSV виде
  (`section,key,value`), удобен для руководства или быстрой выгрузки в Telegram.

Эти эндпоинты доступны по viewer/operator токенам и подходят для ежедневного
контроля риска, а также для внешнего аудита текущего состояния бота.
