...
- Для хаос-тестов добавлены флаги `FEATURE_CHAOS`, `CHAOS_PROFILE`, `CHAOS_WS_DROP_P`, `CHAOS_REST_TIMEOUT_P`, `CHAOS_ORDER_DELAY_MS`, позволяющие имитировать сбои WebSocket/REST и задержки ордеров без изменений кода. Готовые профили заданы в `configs/fault_profiles.yaml`.
...
Для отладки аварийных сценариев биржевые адаптеры можно «зашумлять» без правок кода. Установите `FEATURE_CHAOS=1` и выберите профиль `CHAOS_PROFILE` из `configs/fault_profiles.yaml` (например, `mild`). При необходимости значения можно дополнительно переопределить вручную:
...
Настройки можно задать в `.env`, через раздел `chaos` в `configs/*.yaml` или выбрав готовый профиль. Текущие параметры доступны по `GET /api/ui/chaos`, а на `/ui/dashboard` отображается строка «Chaos profile: ...». Пока `FEATURE_CHAOS=0`, все значения по умолчанию равны нулю, поэтому CI/pytest и рабочие окружения не страдают от случайных сбоев.

## Acceptance suite

Мини-набор проверок для оператора запускается командой `make acceptance` (эквивалент `pytest -m acceptance -q`). Сценарий убеждается, что без хаоса сервис отвечает `200` на `/healthz` и `/live-readiness`, а при включении профиля `mild` бот остаётся в HOLD/safe_mode и вотчдог не деградирует.
...
- Модуль `app/hedge/partial.PartialHedgePlanner` строит план частичного хеджа по остаточным дельтам из risk accounting. Флаги `HEDGE_ENABLED`, `HEDGE_INTERVAL_SEC`, `HEDGE_MIN_NOTIONAL_USDT`, `HEDGE_MAX_NOTIONAL_USDT_PER_ORDER` и `HEDGE_DRY_RUN` управляют запуском цикла (`HEDGE_DRY_RUN=1` — только план без заявок). План сохраняется в `/api/ui/status`, `/api/ui/hedge/plan`, а на `/ui/dashboard` появляется виджет «Partial Hedge» со списком ордеров и кнопкой Execute.
- API `POST /api/ui/hedge/execute` требует payload `{"confirm": true}` и соблюдает двухоператорное подтверждение: при включённом Two-Man Rule необходимы две активные approvals. При ответе биржи `insufficient balance`/`price out of bounds` более трёх раз подряд запускается `engage_safety_hold("partial_hedge:auto_hold")`.

### TCA Router & Cost Model

- В модуле `app/tca/cost_model.py` реализована модель совокупных издержек для перпетуалов: `effective_cost()` суммирует выбранный режим комиссий (maker/taker с учётом VIP-ребейтов), ожидаемый funding на горизонте `horizon_min`, учитывает выбранный VIP-уровень (`TierTable.pick_tier()`) и линейно-квадратичный `ImpactModel`, оценивающий слиппедж в bps/USDT. Хелпер `funding_bps_per_hour()` конвертирует ставки биржи (обычно за 8h окно) в часовые bps, а `FeeTable` и `TierTable` агрегируют параметры по площадкам.
...
- Конфигурация `configs/*.yaml` содержит секцию `tca`: `horizon_min` (дефолтный горизонт в минутах), `impact.k` (чувствительность модели impact) и `tiers[venue]` — массив `{tier, maker_bps, taker_bps, rebate_bps, notional_from}`. Передайте накопленный оборот за 30 дней через `rolling30d=` в API или UI, чтобы выбрать подходящий уровень; при отсутствии значения используется первый (обычно VIP0).
- Предпросмотр TCA доступен из API: `GET /api/ui/tca/preview?pair=BTCUSDT&qty=0.5&rolling30d=750000&book_liq=1500000`. Ответ содержит лучшие направления, детальный разбор по ножкам (включая выбранный tier и impact) и итоговую стоимость в bps/USDT. При включённом флаге на `/ui/dashboard` появляется блок «TCA Preview» с таблицей маршрутов и выделением оптимального направления.
- Поле `impact_bps` в breakdown показывает дополнительный слиппедж в bps, оцененный из `qty/notional` и параметра `book_liq` (USDT ликвидности на стороне стакана). `impact_usdt` — денежный эквивалент этой надбавки. Значение `horizon_min` берётся из секции `tca` по умолчанию, но может быть переопределено в запросе; увеличивайте его для сценариев с удержанием через несколько funding-окон.
- Флаг `FEATURE_SMART_ROUTER=1` включает скоринг `app/router/smart_router.SmartRouter`: выбирается площадка с минимальной совокупной стоимостью (TCA `effective_cost`, дополнительно учтённая `impact_bps*price` и штраф за задержку). Порог задержки берётся из `derivatives.arbitrage.max_latency_ms`, а вес штрафа настраивается переменной `SMART_ROUTER_LATENCY_BPS_PER_MS` (по умолчанию `0.01` bps за миллисекунду превышения). API `GET /api/ui/router/preview?symbol=BTCUSDT&side=buy&qty=1` возвращает лучший venue и подробный breakdown по каждому кандидату, а на `/ui/dashboard` появляется компактная таблица «Router preview» с выделением выбранной площадки.
...
- Новый эндпоинт `GET /api/ui/pnl_attrib` возвращает разложение PnL по стратегиям и
  ...
  Итог считается на одной и той же базе (после фильтрации симуляций):
  `net = realized + unrealized + fees + rebates + funding`, где комиссии
  публикуются со знаком «минус», а ребейты — со знаком «плюс».
...
- Флаг `EXCLUDE_DRY_RUN_FROM_PNL` применяется единообразно: API, UI, ops-report
  ...
  `tracker-adjustment`: она корректирует только `realized`, считается после той же
  фильтрации DRY-RUN и никогда не вычитает симуляционные сделки дважды.
