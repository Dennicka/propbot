...
_SCENARIO_DIR = Path(__file__).with_name("order_router")
_SCENARIOS = sorted(_SCENARIO_DIR.glob("*.json"))


@pytest.mark.parametrize("scenario_path", _SCENARIOS, ids=lambda p: p.stem)
def test_order_router_golden(monkeypatch: pytest.MonkeyPatch, scenario_path: Path) -> None:
    ...
    result = runner.run_order_router()
