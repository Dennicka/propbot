...
- `GET /api/ui/ops_report` (viewer/operator токены) — компактный JSON-срез с
  текущими SAFE_MODE/HOLD и DRY_RUN флагами, статусами стратегий
  (manual enable/disable + risk freeze и причина), экспозицией по venue, списком
  открытых/partial хеджей и текущим unrealised PnL. В блок `recent_actions`
  попадают последние критические действия оператора (hold/resume/disable/
  unfreeze и т.п.).
- `GET /api/ui/ops_report.csv` — тот же набор данных в плоском CSV-формате
  `section,key,value`, удобно для регулярного контроля руководством или быстрой
  выгрузки в Telegram.

Отчёт предназначен для ежедневного риск-контроля и внешнего аудита: даёт
моментальный ответ «в каком режиме бот, что включено, где экспозиция, что
делали операторы» без доступа к сырым логам.
