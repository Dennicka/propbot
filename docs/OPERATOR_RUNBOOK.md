...
3. Изменение параметров на лету (доступно в `paper`/`testnet` при `SAFE_MODE=true`):
   ...
   curl -X PATCH https://<host>/api/ui/control \
     ...
     -d '{
       ...
       "dry_run_only": true,
       "loop_pair": "BTCUSDT",
       "loop_venues": ["binance-um"]
     }'
   ```
   - CLI: `python3 cli/propbotctl.py --base-url https://<host> --token "$API_TOKEN" resume` переключит режим в `RUN` без
     ручного формирования JSON (аналогично `pause`, `rotate-key`, `export-log`).
...
2. Через CLI (рекомендуется при больших объёмах):
   ...
   ```
   Команда требует bearer-токен: передайте его через `--token` либо заранее выставьте переменную окружения `API_TOKEN`.
