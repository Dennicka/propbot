...
- В `.env` задайте `TELEGRAM_ENABLE=true`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID` и `TWO_MAN_TOKEN`. Никогда не публикуйте эти значения и держите чат приватным.
- Бот поддерживает команды `/status`, `/positions`, `/hold <reason>`, `/resume_confirm <token> <note>` и `/kill`.
- `/hold` и `/kill` используют те же внутренние пути, что и REST/UI (`hold_loop`, `engage_safety_hold`, `cancel_all_orders`), поэтому логика хеджа и защиты не дублируется.
- `/resume_confirm` сравнивает `<token>` с `TWO_MAN_TOKEN` и только потом вызывает `approve_resume`. Второе подтверждение всегда делает другой оператор.
- `/status` и `/positions` читают runtime state: SAFE_MODE, HOLD, активные risk breaches, открытые хедж-позиции. Бот не взаимодействует с биржами напрямую.
- Все события пишутся в `data/ops_alerts.json` и доступны через `GET /api/ui/alerts` (read-only, без авторизации) для ретроспективы.
- Любые сбои в доставке алертов не влияют на торговлю: исключения логируются и подавляются.
