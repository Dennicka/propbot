...
- Для повседневной проверки используйте `GET /api/ui/ops_report` — агрегированный
  read-only срез: режим runtime и SAFE_MODE, DRY_RUN/автопилот, очередь RESUME,
  суммарная экспозиция, текущий freeze/enable по стратегиям и последние
  операторские действия/ops-инциденты. Доступен и роли `viewer`, чтобы аудиторы
  могли читать отчёт без прав оператора.
