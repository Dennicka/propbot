...
Для проверки отказоустойчивости без модификации кода используется блок хаос-флагов. При `FEATURE_CHAOS=1` активируются имитации:
...
Параметры читаются из `.env` и раздела `chaos` в YAML-конфиге (`AppConfig.chaos`). Пока `FEATURE_CHAOS=0`, значения игнорируются и прод-окружение/CI работают штатно. 【F:app/config/schema.py†L117-L141】【F:app/services/runtime.py†L594-L630】
