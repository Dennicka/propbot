...
## 6. Testnet smoke workflow

- Workflow **Testnet smoke** доступен в GitHub Actions и запускается вручную (workflow_dispatch) либо по расписанию `0 3 * * *`.
- Джоб выполняется только на ветке `main` (`if: github.ref_name == 'main'`), поэтому в PR он не стартует — требуются секреты с testnet-ключами.
- После выполнения проверяйте артефакт `testnet-smoke-log` — в нём сохраняется сообщение «Smoke placeholder – requires secrets on main» до подключения реальных шагов.

## 7. Funding / Edge Policy
